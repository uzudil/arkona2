...
import cgi, os, json, traceback, commands, base64, re
    ...
    * http://enable-cors.org/
    ...
    def do_POST(self, *args, **kwargs):
        ...
        try:
            ...
            val = postvars['file'][0]
            if 'type' in postvars:
                imgstr = re.search(r'base64,(.*)', val).group(1)
                print imgstr
                print "+++ Saving map=", name, " size=", len(val)
                with open('../assets/world/%s.%s' % (name, postvars['ext'][0]), "wb") as file_:
                    file_.write(base64.decodestring(imgstr))
            else:
                print "+++ Saving map=", name, " size=", len(val)
                with open('../assets/maps/%s.json' % name, "w") as file_:
                    file_.write(val)
